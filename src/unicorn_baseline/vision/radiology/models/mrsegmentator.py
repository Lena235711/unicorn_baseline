...
class MRSegmentator:
    def __init__(self, model_dir: str):
        ...
        n_stages = len(plans['configurations']['3d_fullres']["n_conv_per_stage_encoder"]) #len(self.configuration["n_conv_per_stage_encoder"])
        features_per_stage = [min(plans['configurations']['3d_fullres']["UNet_base_num_features"] * 2 ** i,
                                                    plans['configurations']['3d_fullres']["unet_max_num_features"])
                                                for i in range(len(plans['configurations']['3d_fullres']["n_conv_per_stage_encoder"]))] # self.UNet_max_features_3d = 320, n_stages
        conv_op = nn.Conv3d # len(plans['configurations']['3d_fullres']['patch_size']) == 3 -> nn.Conv3d
        ...
        num_classes = 41
        n_conv_per_stage_decoder = plans['configurations']['3d_fullres']['n_conv_per_stage_decoder']

        self.model = PlainConvUNet(input_channels=input_channels, 
                        n_stages= n_stages, 
